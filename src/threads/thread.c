...
/*load_avg, often known as the system load average, estimates the average
 number of threads ready to run over the past minute. Like cpu_recent_time,
 it is an exponentially weighted moving average*/
static int load_avg;
...
fixed_point to_fixed(fixed_point x);
int to_int_round(fixed_point x);
fixed_point sum(fixed_point x, int n);
fixed_point div_fixed_point(fixed_point x, fixed_point y);
fixed_point mul(fixed_point x, fixed_point y);
fixed_point div(fixed_point x, int n);
...
void thread_init(void)
{
  ...
  initial_thread->thread_nice = NICE_DEFAULT;
  initial_thread->cpu_recent_time = 0;
}
...
void thread_tick(void)
{
  ...
  if (t != idle_thread)
  {
    t->cpu_recent_time = sum(t->cpu_recent_time, 1);
  }
  ...
  if (timer_ticks() % 4 == 0)
  {
    thread_foreach(calculating_priority, NULL);
...
void calculating_priority(struct thread *t, void *aux)
{
  t->priority = PRI_MAX - to_int_round(div(t->cpu_recent_time, 4)) - 2 * t->thread_nice;
}
...
void calculating_load_avg(void)
{
  ...
  load_avg = div(mul(to_fixed(59), load_avg) + to_fixed(ready_threads), 60);
}
...
int thread_get_recent_cpu(void)
{
  return to_int_round(thread_current()->cpu_recent_time * 100);
}

void calculating_recent_cpu(struct thread *t, void *aux UNUSED)
{
  fixed_point decay = div_fixed_point(2 * load_avg, sum(2 * load_avg, 1));
  t->cpu_recent_time = sum(mul(decay, t->cpu_recent_time), t->thread_nice);
}
